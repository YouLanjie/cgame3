...
void Settings() {
	...
	data.addTextData(&data, menuTextDataDescribe, "%s%s", "%z设置游戏更新间隔的秒数%z", "%z设置游戏更新间隔的微秒数%z");
	data.addTextData(&data, menuTextDataSetType, "%s%s", 1, 1);
	data.addTextData(&data, menuTextDataSetVar, "%s%s", &tick.it_interval.tv_sec, &tick.it_interval.tv_usec);
